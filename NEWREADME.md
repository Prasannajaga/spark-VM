...
Run thousands of agent rollouts on a single machine using Firecracker microVMs, without Kubernetes overhead.
...
- Agent rollout execution at scale
- Async RL workloads
...
- Designed for single-host, high-volume rollout execution
- Firecracker microVM isolation per workload
- Host-capacity-aware scheduling
- No cluster orchestration overhead for local execution

## How SparkVM Works

SparkVM runs workloads inside lightweight Firecracker microVMs. Each rollout is isolated, tracked, and managed based on host capacity, which makes large-scale agent rollout execution simpler on a single machine.
...
SparkVM supports both SDK and CLI usage. You can integrate the SDK into your agent systems, rollout pipelines, or automation tools, and use the CLI to trigger and manage rollouts directly from your terminal.
