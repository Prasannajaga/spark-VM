...
class DockifyCommandTest(unittest.TestCase):
    def test_dockify_builds_expected_paths_and_metadata(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sparkvm-dockify-") as tmp:
            ...
            self.assertFalse(validation["shutdown_command_present"])
            self.assertFalse(validation["busybox_present"])
            self.assertIn(
    ...
    def test_dockify_injects_same_universal_init_template_for_any_runtime(self) -> None:
        with tempfile.TemporaryDirectory(prefix="sparkvm-dockify-") as tmp:
            home = Path(tmp)
            ...
            def fake_run_checked(cmd: list[str], *, cwd: Path | None = None):
                del cwd
                if cmd[:2] == ["docker", "create"]:
                    return type("_Result", (), {"stdout": "cid456\n"})()
                if cmd and cmd[0] == "dd":
                    image_path = Path(next(x for x in cmd if x.startswith("of=")).split("=", 1)[1])
                    image_path.parent.mkdir(parents=True, exist_ok=True)
                    image_path.write_bytes(b"disk")
            ...
            def _fake_export(_container_id: str, rootfs_dir: Path) -> None:
                (rootfs_dir / "usr/bin").mkdir(parents=True, exist_ok=True)
                (rootfs_dir / "usr/bin" / "sh").write_text("", encoding="utf-8")
                (rootfs_dir / "usr/bin" / "mount").write_text("", encoding="utf-8")

            def _capturing_write_text(path: Path, content: str, *, encoding: str = "utf-8") -> None:
                if path.name == "init":
                    injected_templates.append(content)
            ...
            with patch("cli.runtimes.shutil.which", return_value="/usr/bin/docker"), patch(
                ...
                "cli.runtimes.write_text", side_effect=_capturing_write_text
            ):
                run_dockify_command(
                    str(home),
                    "node:22-slim",
            ...
            self.assertEqual([SPARKVM_INIT_TEMPLATE], injected_templates)
