    ...
    def _ensure_host_forwarding_rules(self, *, config: NetworkConfig, log_lines: list[str]) -> None:
        ...
        if config.guest_ipv6_cidr:
            ...
            if ipv6_subnet is not None:
                ...
                require_ipv6_forwarding = _env_truthy(os.getenv("SPARKVM_REQUIRE_IPV6_FORWARDING", ""))
                if ipv6_egress_iface is None:
                    message = (
                        "IPv6 forwarding skipped: could not resolve host IPv6 default egress interface. "
                        "Guest IPv4 forwarding remains enabled. Set SPARKVM_REQUIRE_IPV6_FORWARDING=1 to fail instead."
                    )
                    log_lines.append(message)
                    if require_ipv6_forwarding:
                ...
                if shutil.which("ip6tables") is None:
                    message = (
                        "IPv6 forwarding skipped: required host command not found: ip6tables. "
                        "Guest IPv4 forwarding remains enabled. Set SPARKVM_REQUIRE_IPV6_FORWARDING=1 to fail instead."
                    )
                    log_lines.append(message)
                    if require_ipv6_forwarding:
