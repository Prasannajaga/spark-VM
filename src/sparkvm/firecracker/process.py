...
class FirecrackerProcess:
    ...
    def _read_log_tail(self, max_lines: int = 40) -> str:
        if self.log_path is None or not self.log_path.exists():
            return ""
        try:
            lines = read_text(self.log_path, encoding="utf-8", errors="replace").splitlines()
        except OSError:
            return ""
